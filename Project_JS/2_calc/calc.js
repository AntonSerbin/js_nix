...
function func_button(el) {
    ...
    // если прошлое действие было =, то заново набираем цифры, а не продолжаем строку, убираем прошлый знак для повтора
    if (lastActionEqual && (el <= 9 && el >= 0)) action = "";
    if (el !== "=") lastActionEqual = false;

    //если обычное нажатие цифры - добавляем цифру в строку (убираем несколько нулей в пустую строку)
    if (el <= 9 && el >= 0 && afterAction === false) {
        if (!(+string === 0 && +el === 0 && string.length === 1)) string = string + el;
    }

    //если после вычисления - убираем строку и добавляем знак
    if (el <= 9 && el >= 0 && afterAction === true) {
        afterAction = false;
        string = "" + el;
    }
    ...
    if (el === ".") {
        if (string === "" || afterAction === true) string = "0.";
        if (!string.includes(".")) string = string + el;
        afterAction = false;

    }

    //повторяем с помощью = последнее действие
...
function indicateTable(elem) {
    ...
    if (!Number(elem)) stringCalc.innerHTML = elem;
}